...
def test_kp_corrector_points_chamfer():
    """Test points-based keypoint corrector. Currently only ensure it runs without errors."""
    ...
    corrector_node = kp_corrector_reg(cad_models=cad_models, model_keypoints=model_keypoints, solve_tol=1e-4)
    ...
    for i, data in enumerate(dataset_loader):
        ...
        logging.info(f"Naive registration time: {1000 * (end - start) / B} ms")
        ...
        logging.info(f"Corrector time: {1000 * (end - start) / B} ms")
...
def test_kp_corrector_sdf():
    ...
    for i, data in enumerate(dataset_loader):
        ...
        logging.info(f"SDF Corrector time: {1000 * (end - start) / B} ms")
...
def test_kp_corrector_points_chamfer_wolfe_linesearch():
    """Test points-based keypoint corrector. Currently only ensure it runs without errors."""
    logging.basicConfig(level=logging.DEBUG)
    ...
    corrector_node = kp_corrector_reg(
        cad_models=cad_models, model_keypoints=model_keypoints, algo="torch-linesearch-wolfe", log_loss_traj=True,
        solve_tol=1e-4
    )
    corrector = ParamDeclarativeFunction(problem=corrector_node)
