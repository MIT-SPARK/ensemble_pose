...
class CoarseRefinePosePredictor(torch.nn.Module):
    def __init__(self, coarse_model=None, refiner_model=None, bsz_objects=64, cfg=None):
        super().__init__()
    ...
    def batched_model_predictions(self, model, images, K, obj_data, n_iterations=1):
        ...
        for (batch_ids,) in dl:
            ...
            outputs = model(images=images_, K=K_, TCO=TCO_input, n_iterations=n_iterations, labels=labels)
            timer.pause()
    ...
    def make_TCO_init(self, detections, K):
        ...
        if self.cfg['cosypose_coarse_refine']['coarse']['init_method'] == "z-up+auto-depth":
    ...
    def get_predictions(
        self, images, K, detections=None, data_TCO_init=None, n_coarse_iterations=1, n_refiner_iterations=1
    ):
        ...
        if data_TCO_init is None:
            ...
            coarse_preds = self.batched_model_predictions(
                self.coarse_model, images, K, data_TCO_init, n_iterations=n_coarse_iterations
            )
        ...
        if n_refiner_iterations >= 1:
            ...
            refiner_preds = self.batched_model_predictions(
                self.refiner_model, images, K, data_TCO, n_iterations=n_refiner_iterations
            )
    ...
    def forward(self, images, K, detections=None, data_TCO_init=None, n_coarse_iterations=1, n_refiner_iterations=1):
