...
from casper3d.point_transformer import PointTransformerCls, PointTransformerDenseCls
from casper3d.pointnet import PointNetCls
...
class RegressionKeypoints(nn.Module):
    ...
    def __init__(self, N=20, method="point_transformer", local_max_pooling=True, **kwargs):
        super().__init__()
        """
        Inputs: 
        ------
        ...
        if self.method == "point_transformer":
            self.keypoint = PointTransformerCls(
                output_dim=3 * self.N,
                local_max_pooling=self.local_max_pooling,
        ...
        elif self.method == "point_transformer_dense":
            self.keypoint = PointTransformerDenseCls(
                output_dim=3 * self.N, local_max_pooling=self.local_max_pooling, norm_type=kwargs["norm_type"]
            )
