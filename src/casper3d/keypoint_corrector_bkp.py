...
from src.utils.ddn.node import ParamDeclarativeFunction
from src.casper3d.point_set_registration import PointSetRegistration
from src.datasets.shapenet import SE3PointCloud, SE3nIsotropicShapePointCloud, DepthPC
from src.utils.visualization_utils import display_two_pcs, update_pos_tensor_to_keypoint_markers
from src.utils.loss_functions import chamfer_loss, keypoints_loss
from src.utils.evaluation_metrics import shape_error, translation_error, rotation_euler_error
from src.casper3d.pace import PACEmodule
from src.casper3d.model_gen import ModelFromShape
...
# CASPER-3D
class kp_corrector_reg_casper3d:
    def __init__(self, cad_models, model_keypoints, theta=50.0, kappa=10.0, algo='torch', animation_update=False, vis=None):
        super().__init__()
        """
        cad_models      : list of one ModelSDF instances
        model_keypoints : torch.tensor of shape (1, 3, N)
        algo            : 'scipy' or 'torch'
        """
        self.cad_models = cad_models
    ...
    def objective(self, detected_keypoints, input_point_cloud, correction):
        ...
        #ToDo: Move loss_pc computation to src.utils.loss_functions.py
        # model_estimate = R @ self.cad_models + t
        # loss_pc = chamfer_loss(pc=input_point_cloud, pc_=model_estimate, max_loss=True)
        sdf = self.cad_models[0]
        pc = torch.transpose(R, -1, -2) @ (input_point_cloud - t)
        dist = sdf.forward(pc)
        loss_pc = torch.max(dist**2, -1)[0]     # shape: (B, 1)
