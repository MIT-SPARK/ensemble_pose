...
def densefeat_perturbation(densefeat, var=0.8, fra=0.2):
    """
    inputs:
    densefeat  :  torch.tensor of shape (B, 3, n)
    var             :  float
    fra             :  float    : used if type == 'sporadic'

    output:
    densefeat detected  : torch.tensor of shape (B, 3, n)
    """
    device_ = densefeat.device

    mask = (torch.rand(size=densefeat.shape).to(device=device_) < fra).int().float()
    detected_densefeat = densefeat + var * (torch.rand(size=densefeat.shape).to(device=device_) - 0.5) * mask

    return detected_densefeat

lossMSE = torch.nn.MSELoss(reduction="none")

class densefeat_corrector_reg:
    def __init__(
        self,
        cad_models,
        model_features,
        ...
        self.device_ = model_features.device
    ...
    def matching(self, features, pc):
        """
        inputs:
        features    :   torch.tensor of shape (B, d, m)
        pc          :   torch.tensor of shape (B, 3, m)

        output:
        matched pc  :   torch.tensor of shape (B, 3, n)

        """

        model_features = self.model_features    # shape (1, d, n)
        fh = model_features.transpose(-1, -2) @ features
        fh_softmax = torch.nn.functional.softmax(fh, dim=-1)

        points = fh_softmax @ pc.transpose(-1, -2)

        return points.transpose(-1, -2)

    def objective(self, detected_features, input_point_cloud, correction):
        """
        inputs:
        detected_features  : torch.tensor of shape (B, d, m)
        input_point_cloud   : torch.tensor of shape (B, 3, m)
        correction          : torch.tensor of shape (B, d, m)
        ...
        #NOTE: We implement a soft-max feature-based aggregation of points, inspired by Deep Point Matching.
        #   - This truns out to be differentiable
        matched_points = self.matching(detected_features + correction, input_point_cloud)

        R, t = self.point_set_registration_fn.forward(matched_points)
        ...
        #NOTE: can't add norm of corrector in the optimization. The backprop would be incorrect.
        return self.kappa * loss_pc # + self.theta * lossMSE(correction, torch.zeros_like(correction)).sum(1).mean(1).unsqueeze(-1)

    def solve(self, detected_features, input_point_cloud):
        """
        input:
        ...
        if self.algo == "torch-linesearch-wolfe":
            ...
            correction = self.batch_accel_gd(
                detected_features, input_point_cloud, max_iterations=self.max_solve_iters, tol=self.solve_tol
            )
    ...
    def batch_accel_gd(self, detected_features, input_point_cloud, lr=0.1, max_iterations=1000, tol=1e-12, gamma=0.1):
