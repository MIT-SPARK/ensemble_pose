...
def is_close(keypoints, pcd, threshold=0.015):
    ...
    closest_dist, _, _ = ops.knn_points(
        torch.transpose(keypoints, -1, -2), torch.transpose(pcd, -1, -2), K=1, return_sorted=False
    )
...
def translation_error(t, t_):
    ...
    if t.dim() == 2:
        ...
        # check
        _temp = torch.norm(t - t_, p=2, dim=1) / 3.0
        # if torch.any(torch.isnan(_temp)):
        #     breakpoint()
...
def add_s_error(
    predicted_point_cloud, ground_truth_point_cloud, threshold, certi=None, degeneracy_i=None, degenerate=False
):
    """
