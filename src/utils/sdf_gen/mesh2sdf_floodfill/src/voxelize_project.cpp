...
Vox <vtype> voxelize(Eigen::MatrixXf &V, Eigen::MatrixXi &F, float res, vtype w0, vtype w1, int pad, bool verbose) {
  ...
  // min verts coordinates of the vox grid (so we have to subtract the pad * res)
  Eigen::Vector3f vmin = V.rowwise().minCoeff() - Eigen::Vector3f::Constant(pad * res);
  // max verts coordiantes of the vox grid (so we have to add the pad * res)
  ...
  for (int f = 0; f < (int) F.cols(); f++) {
    ...
    // v are the triangle's coordinates in the grid frame
    ...
    float d_xy_e0 = (-1.0f * glm::dot(n_xy_e0, glm::vec2(v0.x, v0.y))) + glm::max(0.0f, res * n_xy_e0[0])
        + glm::max(0.0f, res * n_xy_e0[1]);
    float d_xy_e1 = (-1.0f * glm::dot(n_xy_e1, glm::vec2(v1.x, v1.y))) + glm::max(0.0f, res * n_xy_e1[0])
        + glm::max(0.0f, res * n_xy_e1[1]);
    float d_xy_e2 = (-1.0f * glm::dot(n_xy_e2, glm::vec2(v2.x, v2.y))) + glm::max(0.0f, res * n_xy_e2[0])
        + glm::max(0.0f, res * n_xy_e2[1]);
    // YZ plane
    ...
    float d_yz_e0 = (-1.0f * glm::dot(n_yz_e0, glm::vec2(v0.y, v0.z))) + glm::max(0.0f, res * n_yz_e0[0])
        + glm::max(0.0f, res * n_yz_e0[1]);
    float d_yz_e1 = (-1.0f * glm::dot(n_yz_e1, glm::vec2(v1.y, v1.z))) + glm::max(0.0f, res * n_yz_e1[0])
        + glm::max(0.0f, res * n_yz_e1[1]);
    float d_yz_e2 = (-1.0f * glm::dot(n_yz_e2, glm::vec2(v2.y, v2.z))) + glm::max(0.0f, res * n_yz_e2[0])
        + glm::max(0.0f, res * n_yz_e2[1]);
    // ZX plane
    ...
    float d_xz_e0 = (-1.0f * glm::dot(n_zx_e0, glm::vec2(v0.z, v0.x))) + glm::max(0.0f, res * n_zx_e0[0])
        + glm::max(0.0f, res * n_zx_e0[1]);
    float d_xz_e1 = (-1.0f * glm::dot(n_zx_e1, glm::vec2(v1.z, v1.x))) + glm::max(0.0f, res * n_zx_e1[0])
        + glm::max(0.0f, res * n_zx_e1[1]);
    float d_xz_e2 = (-1.0f * glm::dot(n_zx_e2, glm::vec2(v2.z, v2.x))) + glm::max(0.0f, res * n_zx_e2[0])
        + glm::max(0.0f, res * n_zx_e2[1]);
