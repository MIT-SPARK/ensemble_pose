...
def scale_to_unit_cube(mesh):
    """Scale the given mesh into fitting a unit cube"""
    if isinstance(mesh, trimesh.Scene):
        mesh = mesh.dump().sum()

    # t: how much is the mesh off center from the origin
    # s: max(length, width, height) of the original mesh
    # new_mesh = (original_mesh - t) / (s / 2) = (original_mesh - t) * 2 / s
    # original_mesh = new_mesh * s / 2 + t
    t = mesh.bounding_box.centroid
    s = np.max(mesh.bounding_box.extents)

    vertices = mesh.vertices - t
    vertices *= 2 / s

    return trimesh.Trimesh(vertices=vertices, faces=mesh.faces), s, t
