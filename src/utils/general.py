...
def o3dpc_to_tensor(pc):
    """
    :pc open3d point cloud
    :returns: torch tensor of shape (3, m)
    """

    pc_torch = torch.from_numpy(np.asarray(pc.points)).transpose(0, 1)  # (3, m)
    pc_torch = pc_torch.to(torch.float)

    return pc_torch


def pos_tensor_to_o3d(pos, estimate_normals=True, color=None):
    """
    inputs:
