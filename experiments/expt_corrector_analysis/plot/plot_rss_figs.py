...
def plot_frac_cert(
        kp_noise_var_range, save_fig=True, base_filename="frac_cert", base_folder="./", obj_id=None, **kwargs
):
    """Plot fraction certifiable"""
    certi_naive = kwargs["certi_naive"]
    certi_corrector = kwargs["certi_corrector"]
    certi_icp = kwargs["certi_icp"]
    ...
    if not save_fig:
        ...
        plt_save_figures(f"{base_filename}_{obj_id}", base_folder)
    plt.close()
...
def plot_adds(
        kp_noise_var_range, save_fig=True, adds_base_filename="adds_comp", base_folder="./", obj_id=None, **kwargs
):
    """Plot 3 plots:
    1. ADD-S error
    ...
    chamfer_pose_icp_to_gt_pose = kwargs["chamfer_pose_icp_to_gt_pose"]
    chamfer_metric_icp_var = kwargs["chamfer_metric_icp_var"]
    chamfer_metric_icp_mean = kwargs["chamfer_metric_icp_mean"]
    chamfer_metric_icp_certi_var = kwargs["chamfer_metric_icp_certi_var"]
    chamfer_metric_icp_certi_mean = kwargs["chamfer_metric_icp_certi_mean"]
    ...
    chamfer_metric_icp_certi_var = torch.sqrt(chamfer_metric_icp_certi_var).T
    chamfer_metric_icp_var = torch.sqrt(chamfer_metric_icp_var).T
    ...
    ax_adds.errorbar(
        x=kp_noise_var_range,
        y=chamfer_metric_icp_mean,
        yerr=chamfer_metric_icp_var,
        fmt=":x",
        color="red",
        ecolor="salmon",
        elinewidth=1,
    ...
    ax_adds.errorbar(
        x=kp_noise_var_range,
        y=chamfer_metric_icp_certi_mean,
        yerr=chamfer_metric_icp_certi_var,
        fmt=":o",
        color="orangered",
        ecolor="salmon",
        elinewidth=3,
        capsize=0,
        label="icp + certification",
    )
    ...
    if not save_fig:
        ...
        plt_save_figures(f"{adds_base_filename}_{obj_id}", base_folder)
    plt.close()
...
if __name__ == "__main__":
    ...
    # base_folder = os.path.join(dump_path, "rss_plots", args.object_label)
