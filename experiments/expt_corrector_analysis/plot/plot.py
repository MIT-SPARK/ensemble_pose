...
def certification(data, parameters, epsilon, num_iterations=100, full_batch=False):
    ...
    certify = certifiability(epsilon=epsilon * object_diameter,
                             clamp_thres=chamfer_clamp_thres_factor * object_diameter)

    ###
    certi_naive = torch.zeros(size=(xlen, num_iterations), dtype=torch.bool).to(
        device=device_)
    certi_corrector = torch.zeros(size=(xlen, num_iterations), dtype=torch.bool).to(
        device=device_)
    ...
    for kp_noise_var_i in range(len(sqdist_input_naiveest)):
        ...
        for batch_i in range(len(sqdist_input_naiveest[kp_noise_var_i])):
            ...
            certi_naive_batch = certify.forward_with_distances(
                sqdist_input_naive[0], sqdist_kp_naive, pc_padding)
