...
from cfg import CosyposeCfg
from cosypose.lib3d.rigid_mesh_database import MeshDataBase
from datasets.bop import make_object_dataset, make_scene_dataset
from datasets.pose import PoseDataset, FramePoseDataset
...
def self_supervised_train_one_epoch_frame_objs_depth_only(
    training_loader,
    mesh_db_batched,
    seg_model,
    model,
...
def validate_depth_only(validation_loader, mesh_db_batched, model, cert_model, tensorboard_writer, device, cfg):
...
def train_without_supervision(
    ...
    if cfg["training"]["dataloader_type"] == "frame_objs" and len(cfg["models_to_use"]) > 1:
        train_one_epoch = self_supervised_train_one_epoch_frame_objs
    elif cfg["training"]["dataloader_type"] == "frame_objs" and cfg["models_to_use"][0] == "c3po_multi":
        train_one_epoch = self_supervised_train_one_epoch_frame_objs_depth_only
    else:
        raise NotImplementedError
...
def train_detector(cfg, detector_type="point_transformer", **kwargs):
    ...
    # load scene renderer
    # We don't need renderer for depth only test
    scene_renderer = []

    # load multi model
    model = load_multi_model(batch_renderer=batch_renderer, meshdb_batched=mesh_db_batched, device=device, cfg=cfg)

    if cfg["multimodel"]["load_pretrained_weights"]:
        wpath = cfg["multimodel"]["pretrained_weights_path"]
        logging.info(f"Loading pretrained weights for MultiModel at {wpath}.")
        save = torch.load(wpath)
